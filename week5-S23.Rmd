...
The fitted regression equation was: $\hat{y}_i =$. Plotting the regression equation on top of the scatterplot of the original data indicates there is a problem with the assumption of a linear relationship between the variables length (in inches on a photograph) and weight (in lbs). 
...
Many of you explored this data further and discovered that a logarithmic transformation of the variable *inches* produces a scatterplot with a more linear trend. The fitted regression equation for the transformed data was: $\hat{y}_i = \ln (x_i)$. The plot of the transformed data and fitted regression equation looks like a much better fit for a linear model. 
...
* In the gator model above, the regression model for the original data has a correlation coefficient of $x$ and an $R^2$ value of $x$. The regression model for the transformed data has a correlation coefficient of $x$ and an $R^2$ value of $x$. 

### 2. Formula for the slope of a SLR 

In the gator model above, the slope of the regression model for the original data is $x$ which means that, based on this model, each additional inch of length (in the areal photograph), corresponds to an additional weight of $xlbs$.  


The slope of the regression model for the transformed data is a little more complicated to interpret. The slope of $x$ means that, based on this model, each additional logarithm of an inch of length (in the areal photograph) corresponds to an additional weight of $xlbs$. Since it is difficult to think in terms of logarithm of inches, we would most likely want to report the interpretation of the slope within the context of the original units of the data.  

...
**1.**  An entomologist writes an article in a scientific journal which claims that fewer than $6\%$ of male fireflies are unable to produce light due to a genetic mutation. Identify the Type I error in this context.
...
\vspace{1cm}
**3.**  A candy company claims that its bags of mixed suckers are $20\%$ strawberry, $30\%$ cherry, $15\%$ apple, $10\%$ lemon, and $25\%$ grape. A bag was purchased, and the number of each type of flavor was recorded in the chart below. Determine which statistical test is appropriate for this situation.
...
\vspace{1cm}
**5.**  A certain population is strongly skewed to the right. We want to estimate its mean, so we will collect a sample. Which of the following statements should be TRUE if we use a large sample rather than a small one?
...
\vspace{1cm}
**6.** Using a t-table, estimate the p-value for a test statistic of $1.76$ with $24$ degrees of freedom.
...
\vspace{1cm}
**8.** Consider the five number summary of hourly wages ($) for a sample of advertising/promotion managers. 
...
\vspace{1cm}
**10.** A bicycle shop equips $60\%$ of their bikes with a water bottle holder. $55\%$ of the bikes they sell have a kickstand attached to the bike. $34\%$ of the bikes sold have both features.
...
\vspace{1cm}
**19.** A survey found that $79\%$ of a random sample of 1024 American adults approved of cloning endangered animals. Find the margin of error for this survey if we want $90\%$ confidence in our estimate of the percent of American adults who approve of cloning endangered animals.
...
\vspace{1cm}
**20.** Based on a sample of 30 randomly selected years, a $90\%$ confidence interval for the mean annual precipitation in one city is from 48.7 inches to 51.3 inches. Find the margin of error.    
...
**21.** Ten different families are tested for the number of gallons of water a day they use before and after viewing a conservation video. Given that $\bar{x}_{d} = 4.8$ and $s_d = 5.25$, a $90\%$ confidence interval for the mean of the differences in water usage 

\vspace{1cm}
will be centered at $\rule{3cm}{0.15mm}$ and have a margin of error equal to $\rule{3cm}{0.15mm}$. 

\vspace{3cm}
**22.** Suppose the yearly snowfall (in $cm$) in a certain town can be modeled with a Normal distribution with mean $57cm$ and standard deviation $8 cm$. Based on the Z-table, the (lower) 80th percentile 
...
\vspace{3cm}
**25.**  A manufacturing process has a $70\%$ yield, meaning that $70\%$ of the products are acceptable and $30\%$ are defective. 
...
\vspace{1.5cm}
**29.** In words (not symbols), the parameters of a Normally distributed random variable are
...
\vspace{1cm}
**30.** In words (not symbols), the parameters of a Binomial distributed random variable are
...
\vspace{5cm}
**34.** An education researcher was interested in examining the effect of the teaching method and the effect of the particular teacher on students' scores on a reading test. In a study, there are two different teachers (Juliana and Felix) and three different teaching methods (method A, method B, and method C). The number of students participating in the study is 258. Students are randomly assigned to a teaching method and teacher. 

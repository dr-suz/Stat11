---
title: "Definitions and terms to know - part 3"
output: pdf_document
fontsize: 14pt 
---
...
### Bernoulli 
  ...
  - **Intitutive definition:** For binary categorical variables. Represents a single success or failure. (Leave all preconceived notions of success at the door, it has no positive connotation in statistics.) E.g. a single flip of a coin can either result in heads or tails. (Whichever outcome we consider a "success" is up to us to decide.)

- **Notation:** $X \sim Bern(p)$ 

- **R functions:** `rbinom(n, size=1, prob)` Note a Binomial random variable (below) is just the sum of the number of success of $n$ (independent) Bernouli random variable. 
...
- **Notation:** $X \sim Bin(n,p)$ Here $n$ is the number of trials, or, flips of "a coin" so $n$ is the largest possible number of successes we could observe.
...
- **Sample space:** The sample space $S$ is a bit difficult to visualize (since it's multidimensional) but it consists of a finte number of elements. Each element represents, over these $n$ rolls, how many times did we observe $1$, the first face of our die, how many times did we observe $2$, the second face of our die, $\dots$, how many times did we observe $k$, the last face of our die. 
...
- **Intuitive defintiion:** Randomly choosing any real number (rational or irrational) between two specific numbers.

...
Data consists of observational units (rows) and variables (columns). We covered a few ways to conduct an exploratory data analysis for numeric and/or categorical variables. Here you can find [a couple of examples of what data might look like](https://docs.google.com/spreadsheets/d/1N_gAm3cUKGUSRWOw15zwtQa-eLLw0-OsNsBxdp-s16k/edit?usp=sharing) if we want to conduct a confirmatory analysis of the claim that Swarthmore grads have a higher acceptance rate for grad school compared to other schools. 
...
* <span style="color:green">Mean</span>: $\bar{x} = \frac{1}{n}\sum_{i=1}^{n}x_i = \frac{1}{n}\left(x_1 + x_2 + x_3 + \dots + x_{n-1} + x_{n} \right)$

* <span style="color:green">Mode</span>: the highest peak(s)

* <span style="color:green">Median</span>: 50\% of the data is larger and 50\% of the data is smaller than this value 
...
- <span style="color:green">unimodal</span> (exactly one peak)

- <span style="color:green">symmetric about the mode</span> (mirrored at the mode)

the mode, the mean, and the median are all the same value! 

![](./images/symmetric unimodal.png){width=500px}

For data that is unimodal but not symmetric...

![*The median is a better representation of the center of skewed data.*](./images/skew.png){width=500px}

For data that is not unimodal but is symmetric...

![*It is useful to know how many modes occur and where they are located.*](./images/bimodal symmetric.png){width=500px}

For data that is not unimodal and not symmetric... 

![*Histograms are indespensible because numerical summaries can be misleading by themselves.*](./images/neither symmetric nor unimodal.png){width=500px}

# 2. Numerical measures of spread 

* <span style="color:green">Range</span>: $x_{max} - x_{min}$ 

* <span style="color:green">Interquartile range</span>: the range of two values where 25\% of the data are smaller than the lower number and 75\% of the data are smaller than the higher number  

* <span style="color:green">Variance (or standard deviation)</span>: Average distance from the mean (or square root of this average distance)
...
![](./images/shacarri-richardson.jpg){height=200px}  ![](./images/camryn-rogers.png){height=200px} 

<font size="1">Image credits: Denes Erdos/AP (L) Charlie Crowhurst/Getty Images for IAAF (R)</font>



The 2023 World Athletics Championships in Budapest saw many athletes perform at their personal best in preparation for the Olympics. 
Sha'Carri Richardson won first place for the USA in the women's $100m$ sprint with a time of $10.65s$, which was $0.252 s$ faster than the average time of her top $10$ competitors. Camryn Rogers of Canada won the women's hammer throw with a throw of $77.22 m$, a throw $3.383 m$ longer than the average distance of her top $10$ competitors. 

<span style="color:purple">Statistically speaking, whose performance are we most impressed by?</span>
...
<span style="color:green">Normal/Gaussian model</span>

- unimodal

- symmetric about the mode 

- not too spread out from the mode (almost no data points lie really far away from the mode)
...
![](./images/normal 68-95-997.png){width=700px}

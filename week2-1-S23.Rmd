...
Thiam took the gold in the Olympics with a long jump of $6.58 m$ which was $0.41 m$ longer than the average jump. Johnson-Thompson won the $200 m$ race with a time of $23.26 s$ which was $1.32 s$ faster than the average run. Whose performance are you more impressed by? 
...
The Normal model has two model parameters: $\mu$ the mean and $\sigma$ the standard deviation. These two parameters correspond directly to a measure of centrality and a measure of spread. (This isn't the case with all types of probability models.)
...
Let's check to see if the data plotted above could plausibly com from a Normal distribution: `r data`

The mean of this data is `r mean(data)` and the standard deviation is `r sd(data)`.

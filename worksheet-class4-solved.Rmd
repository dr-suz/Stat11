...
summary_stats <- global_wealth_and_health %>% summarise(data_mean = mean(LifeExpect), data_var=var(LifeExpect), data_sd = sd(LifeExpect))
summary_stats
```
...
```{r}
lower_bound <- summary_stats$data_mean - (3*summary_stats$data_sd)
upper_bound <-summary_stats$data_mean + (3*summary_stats$data_sd)
global_wealth_and_health %>% filter((LifeExpect<=upper_bound)&(LifeExpect>=lower_bound)) %>% count 
```
...
```{r}
global_wealth_and_health2 %>% ggplot(aes(sample=LifeExpectStand)) + stat_qq() + stat_qq_line()
```

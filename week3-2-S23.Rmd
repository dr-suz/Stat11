...
When fitting a SLR model, one numeric variable plays the role of a predictor and the other variable plays the role of the response. This is a statistical model because we are assuming that the equation of a line can explain the relationship between the predictor and response except for some tiny random error, $\varepsilon$:
$$y_i = b + m x_i + \varepsilon.$$ 
...
$$\text{Residual} = e_i = y_i - {\hat{y}}_i = \text{observed value} - \text{predicted value}$$
Informally, you can think of the residuals, $(e_1, e_2, \dots, e_n)$, as possible values for the tiny random error, or <span style="color:green">random noise</span>, $\varepsilon$.  

The SLR model assumes that the random noise, $\varepsilon$ follows a Normal probability distribution (so the 68/95/99.7 rule applies). We investigate properties of the residuals in order to help us assess whether or not this assumption seems reasonable. First, we need to describe the center and spread of the residuals. 
...
Note that $b_1 = \frac{s_{xy}}{\sqrt{s_x s_y}} \cdot \frac{s_y}{s_x} = r \frac{s_y}{s_x}$. Just as in any mathematical linear equation, $b_1$ represents the amount of change we expect in $y$ if we increase $x$ by one unit. Unlike ordinary mathematical formulas however, the slope here represents an **average** amount of change in $y$, not an actual amount. This is because we assume $y$ is a function of a random variable, $\varepsilon$.  
